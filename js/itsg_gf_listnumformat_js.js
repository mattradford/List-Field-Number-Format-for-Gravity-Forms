/**
 * Extract all regex matches from a string into an array of match arrays
...
function itsg_gf_list_number_format_newrow( new_row, row ){
	...
	if ( '1' == itsg_gf_listnumformat_js_settings.is_entry_detail ) {
		var field_parent = row.parents( 'table.gfield_list_container' ).prev( 'input' );
		if ( field_parent.length && field_parent.attr('id') ) {
			var field_details = field_parent.attr('id').split( '_' );
			field_id = field_details[ field_details.length - 2 ];
		}
	} else {
		var field_parent = row.parents( 'li.gfield' );
		if ( field_parent.length && field_parent.attr('id') ) {
			var field_details = field_parent.attr('id').split( '_' );
			field_id = field_details[ field_details.length - 1 ];
		}
	}
	// If we couldn't determine field_id, try fallback methods
	if ( null === field_id ) {
		var field_parent = row.parents( 'table.gfield_list_container' ).prev( 'input' );
		if ( field_parent.length && field_parent.attr('id') ) {
			var field_details = field_parent.attr('id').split( '_' );
	...
	if ( null === field_id ) {
		// If still no field_id, skip processing for this row
		return;
	}
	var form_id = itsg_gf_listnumformat_js_settings.form_id;
...
function itsg_clean_number( value, isNumberRounding, decimalSeparator, thousandSeparator, isNumberFixedPoint, isNumberRoundingDirection ) {
	...
	// Check if value is a number - use gform.utils.isNumber() for GF 2.9+ with fallback
	if ( typeof gform !== 'undefined' && typeof gform.utils !== 'undefined' && typeof gform.utils.isNumber === 'function' ) {
		if ( ! gform.utils.isNumber( value ) ) {
			value = 0;
		}
	} else if ( typeof gformIsNumber === 'function' && ! gformIsNumber( value ) ) {
		value = 0;
